...
Deno.serve(async (req: Request) => {
  console.log('=== NEW DOMAIN ACCESS REQUEST START ===');
  ...
  try {
    ...
    if (!domainId) {
      ...
      return new Response(
        JSON.stringify({ error: 'Domain ID required', code: 400 }),
        { 
          status: 400, 
    ...
    if (!token) {
      ...
      return new Response(
        JSON.stringify({ error: 'Authorization token required', code: 401 }),
        { 
          status: 401, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    if (authError || !user) {
      ...
      return new Response(
        JSON.stringify({ error: 'Invalid authentication token', code: 401 }),
        { 
          status: 401, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    if (domainError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Domain fetch failed', code: 500 }),
        { 
          status: 500, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    if (!domains || domains.length === 0) {
      ...
      return new Response(
        JSON.stringify({ error: 'Domain not found', code: 404 }),
        { 
          status: 404, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    if (accessError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Access check failed', code: 500 }),
        { 
          status: 500, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    if (!userAccess || userAccess.length === 0) {
      ...
      return new Response(
        JSON.stringify({ error: 'Access denied - user not assigned to this domain', code: 403 }),
        { 
          status: 403, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
...
<body>
    ...
    console.log('=== NEW DOMAIN ACCESS SUCCESS ===');
    ...
    console.error('=== NEW DOMAIN ACCESS ERROR ===');
